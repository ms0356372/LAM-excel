...
- 使用 `ttk.Treeview` 的規則表格檢視（順序、啟用、欄位、表頭、性別、判斷方式、條件、分級、狀態）
- 規則搜尋，以及性別、分級、狀態、啟用狀態的組合篩選
- 共用既有衝突檢查結果的規則狀態與警告提示
- 依欄位、性別、實際判斷順序整理的只讀規則總覽（支援搜尋與僅有警告篩選）
- JSON 自動保存、另存與載入
...
規則列表預設嚴格按照規則 JSON／程式內 `rules` 清單的正式順序顯示，不提供會改動資料順序的欄名排序。搜尋可比對欄位、表頭、性別、判斷方式、格式化條件及分級；所有篩選只影響畫面。雙擊表格列會開啟既有的編輯規則視窗，新增、編輯、刪除、啟停及上下移後會立即更新列表。

「規則總覽」是只讀單一視窗，預設僅顯示啟用規則，依來源欄位、性別、原始規則順序分組。它可搜尋顯示文字、只看有警告的規則、篩選性別，亦可選擇顯示停用規則。列表與總覽使用相同的條件格式化函式及既有衝突／覆寫檢查，因此不會建立第二份規則或另一套判斷標準。
...
「匯出 Excel」會依正式的 first match wins 順序匯出全部規則（包含停用規則），不受列表搜尋或篩選影響。`規則` 工作表欄位固定為：啟用、順序、欄位、表頭、性別、判斷方式、下限、上限、判斷值、分級、備註、檢查結果；另附「填寫說明」及格式版本。啟用、性別、判斷方式與分級具有下拉選單。沒有規則時可直接產生 `自訂分級規則範本.xlsx`。

「匯入 Excel」接受上述前十個核心欄位，備註與檢查結果可省略，未知的未來欄位會忽略。空白列也會忽略。程式先檢查格式、數值條件及既有衝突，再顯示逐列預覽和正常／警告／錯誤統計，絕不在選檔後直接修改規則：
...
測試涵蓋區間邊界、嚴格比較、exact match、無效數字、空白、男女優先、共用 fallback、first match wins、停用規則、驗證、JSON 損壞，以及規則條件格式、搜尋、篩選與狀態彙整。Linux CI 無法啟動 Windows Excel COM，因此仍需在 Windows 實機驗證：
...
7. 規則列表搜尋／篩選、雙擊編輯、啟停、刪除及上下移皆即時更新，顯示順序與實際判斷順序一致。
8. 規則總覽只能開啟一個；搜尋、僅有警告、性別與停用規則選項正確，且不修改任何規則。
